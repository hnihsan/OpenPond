...
export default {
  ...
  async getOwnedAssetsByCollection(collection_addr, offset = 0, limit = 20 ) {
    try {
      let response_string = await get_owned_assets_by_collection(collection_addr, String(offset), String(limit));
      let response = JSON.parse(response_string);
      return response.assets
    } catch (error) {
      console.log(error);
  ...
  async getOwnedCollections(owner_address, offset = 0, limit = 20){
    try {
      let response = await get_owned_collections(owner_address, String(offset), String(limit));
